...
const [modal, setModal] = useState(null); // {action, cible, motif_requis}
    ...
    return (
        <div>
            {/* Back + header */}
            <button onClick={() => navigate(estCabinet ? '/demandes' : '/espace-partenaire/demandes')} className="anim-in group inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3.5 py-2 text-sm font-medium text-slate-600 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-deep-blue/40 hover:text-deep-blue hover:shadow-md mb-4">
            ...
            <div className="anim-in card-hover relative overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-b from-slate-50 to-slate-50/60 p-6 md:p-8 mb-6 shadow-sm">
                ...
                <div>
                    <div className="flex items-center gap-3 mb-1.5">
                        ...
                        {nbDevis > 0 && (
                            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-deep-blue-soft text-deep-blue">
                ...
                {estPartenaire(user) && (
                    <button onClick={() => navigate(`${basePath}/demandes/${id}/devis`)}
                        className="flex items-center gap-2 bg-deep-blue hover:bg-deep-blue-dark text-white text-sm font-medium px-4 py-2.5 rounded-lg shadow-sm transition-colors">
            ...
            {estCabinet && demande.transitions?.length > 0 && (
                <div className="anim-in card-hover relative overflow-hidden bg-white border border-slate-100 rounded-2xl p-5 mb-6 shadow-sm">
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                        <div className="mb-2 lg:mb-0">
                            <div className="text-sm font-semibold text-slate-900">Actions disponibles</div>
                            <div className="text-[11px] text-slate-500">Faites évoluer le dossier en un clic</div>
                        </div>
                        <div className="flex flex-wrap gap-2.5 justify-end">
                        {demande.transitions.map((t) => (
                            <button key={t.action} onClick={() => setModal(t)}
                                className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold border shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg ${
                                    t.cible === 'NON_ELIGIBLE' ? 'bg-gradient-to-b from-red-50 to-red-100/70 text-red-700 border-red-200/70 hover:border-red-300 hover:shadow-red-500/20' :
                                    t.cible === 'SANS_SUITE' ? 'bg-gradient-to-b from-slate-50 to-slate-100/70 text-slate-600 border-slate-200 hover:border-slate-300' :
                                    t.cible === 'PIECES_MANQUANTES' ? 'bg-gradient-to-b from-orange-50 to-orange-100/70 text-orange-700 border-orange-200/70 hover:border-orange-300 hover:shadow-orange-500/20' :
                                    'bg-gradient-to-b from-deep-blue to-deep-blue-dark text-white border-transparent hover:shadow-deep-blue/40'
                                }`}>
                                ...
                                {t.cible === 'SOUMISE' && <svg className="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor"><path d="M10.75 4.75a.75.75 0 0 0-1.5 0v4.5h-4.5a.75.75 0 0 0 0 1.5h4.5v4.5a.75.75 0 0 0 1.5 0v-4.5h4.5a.75.75 0 0 0 0-1.5h-4.5v-4.5Z" /></svg>}
            ...
            {/* Devis */}
            {devis.length === 0 ? (
                    <div className="anim-in card-hover bg-white border border-slate-200 rounded-xl p-12 text-center">
                        ...
                        {estPartenaire(user) && (
                            <button onClick={() => navigate(`${basePath}/demandes/${id}/devis`)}
                                className="mt-3 text-sm text-deep-blue hover:text-deep-blue-dark font-medium">+ Créer un devis</button>
                        )}
                    </div>
                ) : (
                    <div>
                        ...
                        <div className="anim-in flex gap-2 overflow-x-auto pb-2 border-b border-slate-100 mb-4">
                            {estCabinet && (
                                <button type="button" onClick={() => { setVueProjet(true); setDevisExpanded(null); }}
                                    className={`flex-shrink-0 inline-flex items-center gap-2 rounded-lg border px-3.5 py-2 text-sm font-medium transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md ${
                                        vueProjet
                                            ? 'border-deep-blue bg-deep-blue text-white shadow-sm hover:shadow-deep-blue/40'
                                            : 'border-slate-200 bg-white text-slate-700 hover:border-deep-blue/50 hover:text-deep-blue'
                                    }`}>
                            ...
                            {devis.map((d) => {
                                ...
                                return (
                                    <button key={d.id} type="button" onClick={() => { setVueProjet(false); setDevisExpanded(d.id); }}
                                        className={`flex-shrink-0 inline-flex items-center px-2.5 py-2 border-b-2 transition-colors ${
                                            actif
                                                ? 'border-deep-blue'
                                         ...
                                         {d.propose_par.logo_url ? (
                                                    <img
                                                        ...
                                                        className="h-[38px] w-auto max-w-[120px] object-contain flex-shrink-0"
                                                    />
                                                ) : (
                            ...
                            {estPartenaire(user) && (
                                <button onClick={() => navigate(`${basePath}/demandes/${id}/devis`)}
                                    className="flex-shrink-0 inline-flex items-center gap-1.5 rounded-lg border border-dashed border-slate-300 px-3.5 py-2 text-sm font-medium text-deep-blue hover:text-deep-blue-dark hover:border-deep-blue/60 transition-colors">
                                    <svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor"><path d="M10.75 4.75a.75.75 0 0 0-1.5 0v4.5h-4.5a.75.75 0 0 0 0 1.5h4.5v4.5a.75.75 0 0 0 1.5 0v-4.5h4.5a.75.75 0 0 0 0-1.5h-4.5v-4.5Z" /></svg>
                                    Nouveau devis
                                </button>
                            )}
                        ...
                        {vueProjet && (
                            <div className="anim-in">
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                    ...
                                    {/* Bloc 1 — Fournisseur */}
                                    <div className="card-hover relative overflow-hidden bg-white border border-slate-100 rounded-xl p-4 shadow-sm">
                                        ...
                                        <h3 className="text-sm font-bold text-deep-blue mb-1">Fournisseur</h3>
                                    ...
                                    <div className="card-hover relative overflow-hidden bg-white border border-slate-100 rounded-xl p-4 shadow-sm">
                                        <span className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[oklch(0.39_0.21_263.59)] via-[oklch(0.48_0.20_262)] to-[oklch(0.52_0.21_27.14)]" />
                                        <div className="text-sm font-bold text-deep-blue mb-3">Détail du demande</div>
                                    ...
                                    <div className="card-hover relative overflow-hidden bg-white border border-slate-100 rounded-xl p-4 shadow-sm">
                                        <span className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[oklch(0.39_0.21_263.59)] via-[oklch(0.48_0.20_262)] to-[oklch(0.52_0.21_27.14)]" />
                                        <div className="text-sm font-bold text-deep-blue mb-3">Document</div>
                                                ...
                                                {demande.documents.map((doc) => (
                                                    <div key={doc.id} className="flex items-start justify-between gap-2 py-2">
                                                        ...
                                                        <button
                                                            ...
                                                            className="text-[10px] font-semibold text-deep-blue hover:text-deep-blue-dark hover:underline flex-shrink-0 mt-0.5"
                                                        >
                                    ...
                                    <div className="card-hover relative overflow-hidden bg-white border border-slate-100 rounded-xl p-4 shadow-sm">
                                        <span className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[oklch(0.39_0.21_263.59)] via-[oklch(0.48_0.20_262)] to-[oklch(0.52_0.21_27.14)]" />
                                        <div className="text-sm font-bold text-deep-blue mb-3">Statut</div>
                                        <div className="divide-y divide-slate-100">
                                            {[
                                                ...
                                                <div key={k} className="flex items-baseline justify-between gap-3 py-2 text-sm last:pb-0">
                                                    <span className="text-slate-500 flex-shrink-0">{k}</span>
                                                    <span className="font-semibold text-slate-900 text-right">{v || '—'}</span>
                                                </div>
                                            ))}
                                    ...
                                    <div className="card-hover relative overflow-hidden bg-white border border-slate-100 rounded-xl p-4 shadow-sm">
                                        <span className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[oklch(0.39_0.21_263.59)] via-[oklch(0.48_0.20_262)] to-[oklch(0.52_0.21_27.14)]" />
                                        <div className="text-sm font-bold text-deep-blue mb-3">Actions</div>
                                        <div className="space-y-4">
                                            <div>
                                                <div className="text-[10px] uppercase text-slate-500 font-medium mb-1">Afficher détail</div>
                                                ...
                                                <button onClick={() => setDetailOpen(true)}
                                                    className="w-full inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium bg-deep-blue text-white hover:bg-deep-blue-dark shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-deep-blue/40">
                                            ...
                                            <div>
                                                <label className="block text-[10px] uppercase text-slate-500 font-medium mb-1.5" htmlFor="precision">Ajouter précision</label>
                                                <textarea id="precision" value={precision} onChange={(e) => setPrecision(e.target.value)}
                                                    ...
                                                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-deep-blue focus:border-deep-blue"></textarea>
                                            ...
                                            <div className="pt-3 border-t border-slate-100 space-y-2">
                                                ...
                                                <button onClick={() => setTacheOpen(true)}
                                                    className="w-full inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium bg-white border border-slate-300 text-slate-700 hover:bg-deep-blue-soft hover:text-deep-blue hover:border-deep-blue/40 transition-colors">
                                                    <svg className="w-4 h-4 text-deep-blue" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M6 5.5A3.5 3.5 0 0 1 9.5 2h1A3.5 3.5 0 0 1 14 5.5v.55c1.7.39 3 1.93 3 3.8v3.4A3.25 3.25 0 0 1 13.75 16H6.25A3.25 3.25 0 0 1 3 12.75v-3.4c0-1.87 1.3-3.41 3-3.8V5.5Zm4 3.5a.75.75 0 0 1 .75.75v2.1l.95.5a.75.75 0 1 1-.75 1.3l-1.5-.8A.75.75 0 0 1 9 12.25v-3A.75.75 0 0 1 9.75 8.5Zm1.75-4.5v.53c.42 0 .83.08 1.2.23A2 2 0 0 0 11.25 4h-.75Z" clipRule="evenodd" /></svg>
                                                    Créer une tâche
                                                </button>
                                            </div>
                                        </div>
                            ...
                            return (
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
                                    ...
                                    {d.propose_par && (
                                        <div className="card-hover relative overflow-hidden bg-white border border-slate-100 rounded-xl p-4 shadow-sm">
                                            <span className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[oklch(0.39_0.21_263.59)] via-[oklch(0.48_0.20_262)] to-[oklch(0.52_0.21_27.14)]" />
                                            <div className="text-sm font-bold text-deep-blue mb-5">
                                                Partenaire
                                            </div>
                                            {/* Logo uniquement */}
                                            <div className="flex items-center gap-3 mb-4">
                                                ...
                                                {d.propose_par.logo_url ? (
                                                    ...
                                                    <div className="w-12 h-12 rounded-xl bg-deep-blue-soft text-deep-blue flex items-center justify-center text-sm font-bold flex-shrink-0">
                                                        {d.propose_par.nom
                                                            ?.split(' ')
                                                            .map((w) => w[0])
                                                            .slice(0, 2)
                                            ...
                                            <div className="divide-y divide-slate-100">
                                                ...
                                                {[
                                                    ['Raison sociale', d.propose_par.nom || '—'],
                                                    ['Type', d.propose_par.type || '—'],
                                                    ['Forme juridique', d.propose_par.forme_juridique || '—'],
                                                    ['SIREN', d.propose_par.siren || '—'],
                                                    ['PAYS', d.propose_par.pays || '—'],
                                                ].map(([k, v]) => (
                                                    <div key={k} className="flex items-baseline justify-between gap-3 py-2 text-sm">
                                                        <span className="text-slate-500 flex-shrink-0">{k}</span>
                                                        <span className="font-semibold text-slate-900 text-right">{v || '—'}</span>
                                    ...
                                    <div className="card-hover relative overflow-hidden bg-white border border-slate-200 rounded-xl p-4 lg:col-span-2">
                                        <span className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[oklch(0.39_0.21_263.59)] via-[oklch(0.48_0.20_262)] to-[oklch(0.52_0.21_27.14)]" />
                                        ...
                                        <div className="flex items-center justify-between gap-3 mb-5">
                                            ...
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 rounded-xl bg-deep-blue-soft text-deep-blue flex items-center justify-center text-sm font-bold flex-shrink-0">
                                                    {d.version || '1'}
                                                </div>
                                                <div>
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-x-6 gap-y-1 mb-4">
                                            <div className="flex items-baseline justify-between gap-2 py-2 border-b border-slate-50">
                                                <span className="text-xs text-slate-500">Prime HT</span>
                                                <span className="font-bold text-slate-900">{fmt(d.prime_ht_cts)}</span>
                                            </div>
                                            <div className="flex items-baseline justify-between gap-2 py-2 border-b border-slate-50">
                                                <span className="text-xs text-slate-500">Taxes</span>
                                                <span className="font-semibold text-slate-900">{fmt(d.taxes_cts)}</span>
                                            </div>
                                            <div className="flex items-baseline justify-between gap-2 py-2 border-b border-slate-50">
                                                <span className="text-xs text-slate-500">Prime TTC</span>
                                                <span className="font-bold text-slate-900">{fmt(d.prime_ttc_cts)}</span>
                                            </div>
                                            <div className="flex items-baseline justify-between gap-2 py-2 border-b border-slate-50">
                                                <span className="text-xs text-slate-500">Frais courtage</span>
                                                <span className="font-semibold text-slate-900">{fmt(d.frais_courtage_cts)}</span>
                                            </div>
                                            <div className="flex items-baseline justify-between gap-2 py-2 border-b border-slate-50">
                                                <span className="text-xs text-slate-500">Première échéance</span>
                                                <span className="font-semibold text-slate-900">{fmt(d.premiere_echeance_cts)}</span>
                                            </div>
                                            <div className="flex items-baseline justify-between gap-2 py-2 border-b border-slate-50">
                                                <span className="text-xs text-slate-500">Fractionnement</span>
                                                <span className="font-semibold text-slate-900">{d.fractionnement || '—'}</span>
                                            </div>
                                            <div className="flex items-baseline justify-between gap-2 py-2 border-b border-slate-50">
                                                <span className="text-xs text-slate-500">Effet possible</span>
                                                <span className="font-semibold text-slate-900">{d.date_effet_possible || '—'}</span>
                                            </div>
                                            <div className="flex items-baseline justify-between gap-2 py-2 border-b border-slate-50">
                                                <span className="text-xs text-slate-500">Valide jusqu'au</span>
                                                <span className="font-semibold text-slate-900">{d.date_validite || '—'}</span>
                                            </div>
                                        </div>

                                        {/* Actions */}
                                        {d.statut === 'ENVOYE' && !d.est_expire && estCabinet && (
                                            <div className="bg-gradient-to-r from-deep-blue-soft to-indigo-50 border border-deep-blue/15 rounded-lg px-4 py-3 flex items-center justify-between gap-3">
                                                <div className="text-[10px] uppercase text-deep-blue font-semibold">Actions</div>
                                    ...
                                    <div className="card-hover relative overflow-hidden bg-white border border-slate-100 rounded-xl p-4 shadow-sm">
                                        ...
                                        <div className="divide-y divide-slate-100">
                                            {[
                                                ['Statut du devis', d.statut === 'DEVIS_SIGNE' ? 'Devis signé' : d.statut],
                                                ['Devis envoyé le', fmtDate(d.date_envoye)],
                                                ['Mis à jour le', fmtDate(d.maj_le)],
                                            ].map(([k, v]) => (
                                                <div key={k} className="flex items-baseline justify-between gap-3 py-2 text-sm last:pb-0">
                                                    <span className="text-slate-500 flex-shrink-0">{k}</span>
                                                    <span className="font-semibold text-slate-900 text-right">{v || '—'}</span>
                                    ...
                                    <div className="card-hover relative overflow-hidden bg-white border border-slate-100 rounded-xl p-4 shadow-sm">
                                        ...
                                        <div className="flex items-center justify-between mb-3">
                                            <div className="text-sm font-bold text-deep-blue">Documents</div>
                                                    ...
                                                    <div key={doc.id} className="flex items-start justify-between gap-2 py-2">
                                                        ...
                                                        <div className="flex items-center gap-1.5 flex-shrink-0 mt-0.5">
                                                            <button
                                                                onClick={() => telechargerDocument(doc.id)}
                                                                title="Télécharger"
                                                                className="w-7 h-7 rounded-lg bg-deep-blue-soft text-deep-blue hover:bg-deep-blue/20 flex items-center justify-center transition-colors"
                                                            >
                                                                <svg className="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor"><path d="M10.75 2.75a.75.75 0 0 0-1.5 0v8.614L6.295 8.235a.75.75 0 1 0-1.09 1.03l4.25 4.5a.75.75 0 0 0 1.09 0l4.25-4.5a.75.75 0 0 0-1.09-1.03l-2.955 3.129V2.75Z" /><path d="M3.5 12.75a.75.75 0 0 0-1.5 0v2.5A2.75 2.75 0 0 0 4.75 18h10.5A2.75 2.75 0 0 0 18 15.25v-2.5a.75.75 0 0 0-1.5 0v2.5c0 .69-.56 1.25-1.25 1.25H4.75c-.69 0-1.25-.56-1.25-1.25v-2.5Z" /></svg>
                                                            </button>
                                                            <button
                                    ...
                                    <div className="card-hover relative overflow-hidden bg-white border border-slate-200 rounded-xl p-4 min-h-[120px]">
                                        <span className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[oklch(0.39_0.21_263.59)] via-[oklch(0.48_0.20_262)] to-[oklch(0.52_0.21_27.14)]" />
                                        <div className="text-sm font-bold text-deep-blue mb-3">Informations supplémentaires</div>
                                                ...
                                                <div className="flex flex-wrap gap-1.5">
                                                    {d.garanties.map((g, i) => (
                                                        <span key={i} className="inline-flex items-center gap-1 text-xs bg-slate-50 border border-slate-200 rounded-full px-2.5 py-1">
                                                            ...
                                                            <span className={`text-[10px] px-1 rounded ${g.optionnelle ? 'bg-deep-blue-soft text-deep-blue' : 'bg-emerald-100 text-emerald-700'}`}>
                                ...
                                {d.conditions_particulieres && (
                                    <div className="card-hover relative overflow-hidden bg-white border border-slate-100 rounded-xl p-4 shadow-sm mb-4">
                                        <span className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[oklch(0.39_0.21_263.59)] via-[oklch(0.48_0.20_262)] to-[oklch(0.52_0.21_27.14)]" />
                                        <div className="text-sm font-bold text-deep-blue mb-2">Conditions particulières</div>
                    ...
                    <div className="bg-white rounded-2xl p-5 w-full max-w-xl shadow-2xl">
                        <div className="flex items-center gap-3 mb-4">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                                ...
                                modal.cible === 'PIECES_MANQUANTES' ? 'bg-orange-100' : 'bg-deep-blue-soft'
                            }`}>
                                <svg className={`w-5 h-5 ${
                                    modal.cible === 'NON_ELIGIBLE' || modal.cible === 'SANS_SUITE' ? 'text-red-600' :
                                    modal.cible === 'PIECES_MANQUANTES' ? 'text-orange-600' : 'text-deep-blue'
                        ...
                        {modal.motif_requis && (
                            <div className="mb-4">
                                ...
                                <textarea className="w-full border border-slate-300 rounded-lg p-3 text-sm focus:ring-2 focus:ring-deep-blue focus:border-deep-blue" rows={3}
                        ...
                        <div className="flex justify-end gap-2 mt-5">
                            ...
                            <button onClick={executeTransition} disabled={busy || (modal.motif_requis && !motif.trim())}
                                className={`px-4 py-2.5 rounded-lg text-sm font-medium text-white shadow-sm disabled:opacity-50 transition-colors ${
                                    ...
                                    'bg-deep-blue hover:bg-deep-blue-dark'
                                }`}>Confirmer</button>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[92vh] overflow-y-auto">
                        ...
                        <div className="flex justify-between gap-2 px-6 pb-6">
                            <button onClick={() => window.print()}
                                className="px-4 py-2.5 rounded-lg text-sm font-medium bg-deep-blue text-white hover:bg-deep-blue-dark shadow-sm transition-colors">
